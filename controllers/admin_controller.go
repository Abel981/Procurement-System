...
func ApproveRequistion(c echo.Context) error {
	...
	approveReqDto := new(dtos.ApproveReqDto)
	...
	_ = departmentAdminCollection.FindOne(ctx, bson.M{"departmentid": updatedDocument.DepartmentId}).Decode(&departmentAdmin)
	fmt.Println(departmentAdmin)
...
func ApproveBid(c echo.Context) error {
	...
	_ = userCollection.FindOne(ctx, bson.M{"_id": updatedBid.SupplierId}).Decode(&approvedSupplier)

	fmt.Println("hey u")
	fmt.Println(approvedSupplier)
	// var approvedSupplier models.User
	// err = userCollection.FindOne(ctx, bson.M{"_id": updatedBid.SupplierId}).Decode(&approvedSupplier)
	services.SendEmail(services.EmailRecipientData{
		FirstName: approvedSupplier.FirstName,
		LastName:  approvedSupplier.LastName,
...
func RejectBid(c echo.Context) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	bidId := c.Param("bidId")
	objId, err := primitive.ObjectIDFromHex(bidId)
	...
	_ = userCollection.FindOne(ctx, bson.M{"_id": updatedBid.SupplierId}).Decode(&rejectedSupplier)
	...
	return c.JSON(http.StatusOK, map[string]string{"message": "Bid rejected successfully"})
}

func ApproveBidManually(c echo.Context) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	reqId := c.Param("reqId")
	...
	cursor, err := bidCollection.Find(ctx, bson.M{"requistionId": objId})
	if err != nil {
		fmt.Println("h1")
		return c.JSON(http.StatusInternalServerError, responses.UserDataResponse{Message: "error", Data: &map[string]interface{}{"data": err.Error()}})
	}
	...
	for cursor.Next(ctx) {
		...
		if bid.Status != "denied" && (bid.Price < lowestBid.Price || lowestBid.ID.IsZero()) {
			lowestBid = bid
		}

	}

	if lowestBid.ID.IsZero() {
	...
	go func() {
		defer wg.Done()
		var approvedSupplier models.User
		err = userCollection.FindOne(ctx, bson.M{"_id": lowestBid.SupplierId}).Decode(&approvedSupplier)
		services.SendEmail(services.EmailRecipientData{
			FirstName: approvedSupplier.FirstName,
			LastName:  approvedSupplier.LastName,
			Email:     approvedSupplier.Email,
	...
	// Start goroutines to update status and send emails to rejected bids
	cursor, err = bidCollection.Find(ctx, bson.M{"_id": bson.M{"$ne": lowestBid.ID}, "requistionId": bson.M{"$eq": lowestBid.RequistionId}})
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": err.Error()})
	}
	defer cursor.Close(ctx)
	...
	update := bson.M{"$set": bson.M{"endDate": time.Now().UTC()}}
	_, err = requisitionCollection.UpdateOne(ctx, filter, update)
...
func ApproveGigRequistion(c echo.Context) error {
	...
	updatedResult, err := gigRequisitionCollection.UpdateOne(ctx, filter, update)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, responses.UserDataResponse{Message: "error", Data: &map[string]interface{}{"data": err.Error()}})
	}

	var updatedDocument models.GigRequistion
	if updatedResult.ModifiedCount > 0 {
		err = gigRequisitionCollection.FindOne(ctx, filter).Decode(&updatedDocument)
		if err != nil {
...
func GetAllGigRequisitions(c echo.Context) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	var requistion []models.GigRequistion
	...
	if len(requistion) == 0 {
		return c.JSON(http.StatusOK, []models.GigRequistion{})
	}

	return c.JSON(http.StatusOK, requistion)
}

func GetGigRequisitionById(c echo.Context) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	...
	var requisition models.GigRequistion
	err = gigRequisitionCollection.FindOne(ctx, bson.M{"_id": objId}).Decode(&requisition)
	if err != nil {
		if err == mongo.ErrNoDocuments {
			return c.JSON(http.StatusNotFound, responses.UserDataResponse{Message: "user not found", Data: nil})
		}
