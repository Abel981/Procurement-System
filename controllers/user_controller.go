...
var gigRequisitionCollection *mongo.Collection = configs.GetCollection(configs.DB, "gigsRequisition")
...
func CreateBid(c echo.Context) error {
	...
	if time.Now().After(requisition.EndDate) {
		fmt.Println("hey 3")
		return c.JSON(http.StatusBadRequest, map[string]interface{}{"message": "Auction deadline has passed"})
	}
	

	var totalPrice = bid.Price * float64(requisition.Quantity)
	isEnough, err := utils.IsBudgetEnough(ctx, requisition.DepartmentId, float32(totalPrice))
...
func GetPasswordResetCode(c echo.Context) error {
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			return c.JSON(http.StatusNotFound, responses.UserDataResponse{Message: "Incorrect email or password", Data: nil})
		}
		return c.JSON(http.StatusInternalServerError, responses.UserDataResponse{Message: "error", Data: &map[string]interface{}{"error": err.Error()}})
	}
